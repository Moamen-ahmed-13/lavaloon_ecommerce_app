...
//   Future<Map<dynamic,dynamic>> _createPaymentIntent(double total) async {
...
//     try {
//       // Await async getCart (returns List<Map<dynamic,dynamic>>)
//       final cartItems =
//           await cartRepository.getCart(); // FIXED: List<Map> from repo
...
//       final validItems =
//           cartItems.where((item) => item['product'] != null).toList();
...
//           taxRate: 0.1); // Example with 10% tax (adjust)

//       // FIXED: Extract List<Product> from maps (safe cast, skip null)
//       final orderItems = validItems
//           .map((item) => item['product']
//               as Product) // FIXED: Use ['product'] for Map, not .product
//           .where((p) =>
//               p != null &&
//               p.id.isNotEmpty) // FIXED: Filter null/invalid products
//           .cast<Product>()
//           .toList();
...
//       print(
//           '✅ Order confirmed: ${order.id}, total: $total, items: ${orderItems.length}'); // Debug log
